...
- **Batch Processing**: Smart edge trimming, background removal, asset renaming, and batch resize (keep aspect ratio + transparent padding to fixed size).
...
- **多语言支持**: 完美支持中文和英文。
- **实时预览**: 导出前提供精确的视觉反馈。
- **批量处理**: 支持智能边缘裁剪、去背景、资产重命名，以及批量调整大小（保持宽高比 + 透明补边到固定尺寸）。
